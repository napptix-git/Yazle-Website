...
function endGame() {
    ...
    setTimeout(() => {
        ...
        showGameOverDiv();
    }, 650); // Delay showing Game Over div by 0.3 sec
...
function animateFall(item) {
    ...
    // Random tilt angle (30°, 45°)
    const tiltAngle = [30, 45][Math.floor(Math.random() * 2)];
    
    // Scale down the item to 0.6
    item.style.transform = `rotate(${tiltAngle}deg) scale(0.5)`;
...
function showGameOverDiv() {
    ...
    textDiv.innerHTML = `You collected <strong>${score}</strong> Chyawanprash bottles!`;
...
function showOverlayEffect() {
    ...
    overlay.src = "./assets/overlay2.png"; // Path to overlay image
