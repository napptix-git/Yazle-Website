    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        productImage.style.cssText = `
          ...
          object-fit: contain;
          transform: scale(0.5);
          transition: transform 0.8s cubic-bezier(0.34, 1.56, 0.64, 1), max-width 0.5s ease, max-height 0.5s ease;
        `;
        ...
        // Blue lighting effect function
        function flashBlueLighting() {
          // Flash the entire overlay blue
          const flash = document.createElement('div');
          flash.style.cssText = `
            position: fixed;
            inset: 0;
            background: radial-gradient(circle, rgba(0,150,255,0.6) 0%, rgba(0,150,255,0.3) 50%, transparent 100%);
            z-index: 10001;
            pointer-events: none;
          ...
          if (!document.getElementById('flashStyle')) {
            ...
            style.textContent = `
              ...
              @keyframes canGlow {
                0%, 100% { filter: drop-shadow(0 0 0px #0096ff); }
                50% { filter: drop-shadow(0 0 30px #0096ff) drop-shadow(0 0 60px rgba(0,150,255,0.5)); }
              }
            `;
            document.head.appendChild(style);
          }
            ...
            if (isOverCan || isOverDropZone || (dropTarget && (dropTarget.id === 'productCan' || dropTarget.id === 'dropZone' || dropTarget.closest('#dropZone')))) {
              ...
              if (droppedPins === 4) {
                ...
                setTimeout(() => {
                  ...
                  // Show Thumbs Up CTA button
                  const thumbsUpButton = document.createElement('a');
                  thumbsUpButton.href = 'https://www.amazon.in/Thums-Up-Tin-300ml/dp/B07G1CYKQV';
                  thumbsUpButton.target = '_blank';
                  thumbsUpButton.style.cssText = `
                    ...
                    letter-spacing: 1px;
                  `;
                  thumbsUpButton.textContent = 'Get Thumbs Up';

                  // Add click tracking
                  thumbsUpButton.addEventListener('click', () => {
                    trackLoopMeEvent('clickthrough', { url: 'https://www.amazon.in/Thums-Up-Tin-300ml/dp/B07G1CYKQV' });
                  });

                  // Hover effect
                  thumbsUpButton.addEventListener('mouseenter', () => {
                    thumbsUpButton.style.transform = 'translateX(-50%) scale(1.05)';
                    thumbsUpButton.style.boxShadow = '0 12px 30px rgba(0, 150, 255, 0.6)';
                  });

                  thumbsUpButton.addEventListener('mouseleave', () => {
                    thumbsUpButton.style.transform = 'translateX(-50%) scale(1)';
                    thumbsUpButton.style.boxShadow = '0 8px 20px rgba(0, 150, 255, 0.4)';
                  });
              ...
              if (droppedPins <= 4) {
                ...
                // Make can smaller after first pin drop
                if (droppedPins === 1) {
                  productImage.style.maxWidth = '43%';
                  productImage.style.maxHeight = '53%';
                }
