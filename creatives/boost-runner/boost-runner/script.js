...
const speedIncreasePerCollect = 0.02;
...
function spawnEntity() {
  ...
  let newY = -60;
  let entityType = Math.random() < 0.65 ? 'collectible' : 'obstacle';
  let tries = 0;
  let maxTries = 10;
  let entity = {
    x: newX,
    ...
    image: entityType === 'collectible' ? burgerImage : obstacleImage
  };
  while (
    (entityType === 'collectible' && isOverlapping(entity, obstacles)) ||
    (entityType === 'obstacle' && isOverlapping(entity, collectibles))
  ) {
...
function checkCollisions(player) {
  ...
  for (let i = collectibles.length - 1; i >= 0; i--) {
    ...
    // Use a higher overlap threshold for fast movement
    if (isCollidingWithRect(playerRect, collectibleRect, 0.3)) {
      createSparkles(
...
function checkObstacleCollisions(player) {
  ...
  for (let i = obstacles.length - 1; i >= 0; i--) {
    ...
    if (isCollidingWithRect(playerRect, obstacleRect, 0.6)) {
      // End the game on collision with obstacle
      gameOver();
      return;
    }
  }
...
function onMouseDown(e) {
  isDragging = true;
  offsetX = e.clientX - (parseInt(character.style.left) || (window.innerWidth / 2));
...
function onTouchStart(e) {
  ...
  offsetX = touch.clientX - (parseInt(character.style.left) || (window.innerWidth / 2));
}
...
window.addEventListener('keydown', function(e) {
  ...
  let left = parseInt(character.style.left) || (window.innerWidth / 2);
  if (e.key === 'ArrowLeft') {
    left -= CHARACTER_MOVE_STEP;
...
function gameLoop() {
  ...
  // Use a smaller, centered hitbox for collision
  const hitboxWidth = charWidth * 0.5;
  const hitboxHeight = charHeight * 0.6;
  const player = {
    x: (parseInt(charElem.style.left) || boardWidth / 2) + (charWidth - hitboxWidth) / 2,
    y: boardHeight - charHeight + (charHeight - hitboxHeight) / 2,
    width: hitboxWidth,
    height: hitboxHeight,
  };
