...
const progressThreshold = 100; // Maximum score for the progress bar
...
function updateCollectibles(characterElem) {
  for (let i = collectibles.length - 1; i >= 0; i--) {
    ...
    const aspectRatio = collectible.image.width / collectible.image.height;
    let drawWidth = collectible.width;
    let drawHeight = collectible.height;

    // Calculate the final width and height preserving aspect ratio
    if (aspectRatio > 1) {
...
function endGame() {
  ...
  // Move all collectibles off-screen
  collectibles.forEach((collectible) => {
    collectible.x = -collectible.width;
  });

  // Step 1: Start skating-right animation
  charachter.classList.add("skating-right");

  // Step 2: Wait for animation to complete
  setTimeout(() => {
    // Step 3: Immediately hide the character after skating
    charachter.style.display = "none";

    // Step 4: Show game over card after 1.5s
    setTimeout(() => {
      showGameOverDiv();
    }, 1500);
  }, 4000); // match skating-right animation duration
}
...
function showGameOverDiv() {
  ...
  textDiv.innerHTML = `<strong>Level Up Your Thirst with Pepsi!</strong>`;
